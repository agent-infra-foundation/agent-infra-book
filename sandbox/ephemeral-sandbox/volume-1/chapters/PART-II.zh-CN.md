# 第 II 部分 — 共享历史与 Workspace Session
...
工具调用
    ↓
基于稳定项目修订的 private workspace session（“私有工作空间会话”）
    ↓
命令执行与私有文件系统变更
    ↓
发布或拒绝
...
因此，隔离单元不是“每个智能体一个文件夹”，而是**每个边界明确的文件系统工作单元一个 workspace session**。

### Automatic Workspace：一条独立命令

当 `exec_command` 请求不包含 `workspace_session_id` 时，运行时会创建 automatic workspace session，并采用固定的 finalization policy（“收尾策略”）`publish_then_destroy`：
...
传入 `workspace_session_id: S18`，会让文件与命令操作使用同一份 mounted project view（“已挂载项目视图”）。C33 可以看到 C32 之前完成的工作，其他 session 则看不到。

C32 结束时不会发布或销毁 S18。multi-operation task（“多操作任务”）准备好后，由 workspace owner（“工作空间所有者”）决定何时收尾，或何时丢弃私有状态。

![Automatic command 获得临时 workspace，在销毁前发布或拒绝；explicit workspace 则跨越一次编辑和两条命令持续存在，直到主动 finalization。](../assets/diagrams/part-2/14-01-automatic-vs-explicit-sessions.svg)

*图 12.1 — 独立命令使用 automatic session；彼此相关的操作主动复用一个 explicit workspace。*

### Session ID 选择一套 Lifecycle
...
添加 session ID 并非无关紧要的路由装饰。它既会改变操作可以观察哪份状态，也会改变修改采用什么生命周期。

不带 session 的读取会看到最新 published snapshot；S18 中的读取会看到它租用的 base 加上 S18 的私有编辑。不带 session 的 file write 会直接发布一个小型 operation-attributed layer（“操作归属层”）；S18 中的写入则保持私有，让智能体可以先编辑、测试和修订，再决定是否发布。
...
| Workspace session | 有边界的任务生命周期 | 稳定 base、私有文件与 finalization |
| Command session | 进程生命周期 | 一个进程及其 transcript |
...
在 v1 中，explicit workspace 的创建与 finalization 属于 internal coordination surface（“内部协调接口面”），而不是公开 CLI 或 MCP tool。公开运行时调用可以指定已有 session ID；外围生命周期由 coordination layer（“协调层”）拥有。

### Task Boundary 优于 Agent Boundary

一个模型进程可能处理多项互不相关的任务。如果把它们保存在同一个 agent-owned folder（“智能体所有文件夹”）中，一个请求的 parser 输出就可能影响另一个请求的 benchmark。独立 automatic session 可以移除这种意外依赖。

反过来也一样。一项修复可能需要一次编辑和多次测试。如果每次调用都得到互不相关的工作空间，编辑就会在命令之间消失。explicit session 可以保留这份有意建立的依赖。

ownership rule（“归属规则”）是：

> *一个 workspace session 属于一个边界明确的文件系统工作单元。这个单元可以是一条独立命令，也可以是数项主动关联的操作。*

orchestrator（“编排器”）可以把任务转交给不同模型，同时保留相同的 workspace identity。reviewer（“审查者”）可以检查 session 而不成为它的作者。“智能体 B 的文件夹”会变成更准确的“基于 R42、服务于这项任务的 workspace S18”。
...
现在，工作单元已经清楚。下一个问题是：所有这些临时 session 从什么状态开始？
...
Ephemeral Sandbox 把持久项目历史保存在一个 **LayerStack** 中。每个 workspace session 租用一个已经记录的 revision（“修订”），再增加自己的 private delta（“私有增量”）：
...
它也比为每项任务复制仓库更精确。复制目录可以提供私有文件，却不说明这些文件来自哪一份共享 revision。workspace session 会把这份 revision 与任务、私有状态、命令和 finalization result 绑定在一起。

### Lease 保持起始 Revision 稳定

S18 从 R42 启动时，LayerStack 会为它提供 snapshot（“快照”）与 **lease**。lease 指定构成 session base 的有序历史，并在 session 存活期间保持这份历史可用。
...
lease 不是全局项目锁。它不会阻止其他 session 发布，也不会阻止新 session 从 R43 开始。它保护的是一个 reader（“读取者”）脚下的稳定地板，并防止清理回收该 reader 仍然需要的 layer。
...
三个 session 可以同时保持活跃。每个 session 都会看到 lease 时间点的完整有序历史，以及自己的 private delta。没有任何 session 会重写下方的 layer。
...
*图 13.1 — 并发 session 可以保留不同但稳定的 LayerStack revision。*

### 共享 Base 只消除一项成本

当 N 个 session 从相同历史开始时，LayerStack 可以避免产生 `N × base` 份仓库副本。每个 session 仍然需要为自己的元数据和 COW 变更付出成本。
...
> **历史是共享的。** Published layer 可以服务于许多 session。
...
![一份不可变 LayerStack 为三个独立的 private workspace session 提供历史；每个 session 只有一个小型 private delta，被接受的发布则向共享历史返回一个新 layer。](../assets/diagrams/part-2/12-01-one-layerstack-many-sessions.svg)

*图 14.1 — 一个 LayerStack 为许多 private session 提供稳定历史；只有被接受的工作才会作为新的共享 layer 返回。*
