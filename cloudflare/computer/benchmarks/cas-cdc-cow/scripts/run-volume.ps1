...
$benchmarkOutput = cmd.exe /d /c "npm.cmd run bench:volume 2>&1"
if ($LASTEXITCODE -ne 0) {
  throw "High-volume benchmark failed with exit code $LASTEXITCODE"
}
$benchmarkOutput | ForEach-Object { Write-Host $_ }

$marker = "VOLUME_BENCHMARK_JSON:"
...
[System.IO.File]::WriteAllText(
  (Join-Path (Get-Location) "results/volume-latest.json"),
  $json,
  [System.Text.UTF8Encoding]::new($false)
)
npm.cmd run report:volume
